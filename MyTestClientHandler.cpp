...
while(!end) {
    ...
    if (ch->keyExistInCache(bufferIn)) {
        ...
        Solver<string, string> *s = getSolver();
...
MyTestClientHandler::MyTestClientHandler(CacheManager<string, string> *cm, Solver<string, string> *solver) : cm(cm), solver(solver) {}

CacheManager<string, string> *MyTestClientHandler::getCm() const {
    return cm;
}

Solver<string, string> *MyTestClientHandler::getSolver() const {
    return solver;
}
